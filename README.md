...
AI Hub 「반려동물 피부 질환 데이터」(dataSetSn=561)로 **정상/이상을 가리고,
이상이면 병변 형태를 알려주는** 딥러닝 파이프라인.

> ⚠️ **이 프로젝트는 수의학적 진단 도구가 아닙니다.**
> 보호자에게 "이런 소견이 의심되니 병원에 가보세요" 수준의 안내만 제공하는
> 스크리닝 보조 기능입니다. 수의사의 진료를 대체하지 않습니다.

> ### ⚠️ **6종 이름은 화면에 안 띄웁니다** (2026-08-26 확정)
>
> holdout 에서 6종 이름이 **46% 틀립니다.** 그래서 6종은 **확률 분포로만**
> 보여주고, 확신이 있을 때 **형태 계열 네 묶음** 중 하나를 말합니다.
> 확신이 낮으면 **아무 이름도 안 말하고** 진료를 권합니다.
>
> | 지금 상태 (2026-09-08) | |
> |---|---|
> | 1단계 병변 검출률 | **94.3%** (holdout) |
> | 계열 4군 커버리지 | **67.9%** — ⚠️ **라벨 네모로 잰 상한** |
> | 실사용 네모로는 | 커버리지가 **상대 −55%** (STEP 39) |
>
> ⚠️ **숫자를 인용할 땐 [`STATUS.md`](STATUS.md) 를 먼저 보세요** —
> 하한선과 단서가 같이 적혀 있습니다.

**현재 위치와 결론:** [`STATUS.md`](STATUS.md) · [`CLAUDE.md`](CLAUDE.md) ·
[`HISTORY.md`](HISTORY.md) — 아래 "빠른 시작" 은 **처음 데이터를 받을 때**의
안내입니다.
