...
def test_readonly_source_is_linked_not_copied():
    ...
    env.load_prepared(src, dest=w)
    crops = w / "crops"
    check("크롭은 링크로 연결된다 (복사 아님)", crops.is_symlink(),
          "복사되었습니다 — Kaggle 20GB 제한에 걸립니다")
    if crops.is_symlink():
        check("링크가 원본을 가리킨다", crops.resolve() == (src / "crops").resolve())
...
if __name__ == "__main__":
    ...
    for fn in [test_finds_extracted_dir, test_readonly_source_is_linked_not_copied,
               ...
               test_missing_gives_useful_error,
