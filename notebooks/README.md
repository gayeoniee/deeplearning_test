...
| **`15_ROI_crop_비교`** | `dogskin-roi` (Private) | 1단계 ×2 (EfficientNetV2-S) | ~3시간 | ★ **대기** — STEP 44 후속. 고정 ROI vs 보존 random ROI |

⚠️ **`03f` · `03g` · `03h` · `14` 는 채택되지 않았거나 미완입니다** (03f 축 닫힘,
03g 헛알림 +1.8%p, 03h 는 재개 조건이 발동 안 함 — STEP 23, `14` 는 이득 없음 —
STEP 44). 파일은 **남겨둡니다** — 기각도 결과라 다시 묻지 않으려면 남아 있어야
합니다. 결론은 [`docs/results/`](../docs/results/) 의
`STEP14_1단계_백본_판A_실측.md` · `STEP15_털가중샘플러_실측.md` ·
`STEP44_병변보존_random_crop_실측.md` 에 있습니다.
